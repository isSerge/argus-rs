...
pub async fn upload_abi(
    ...
    let abi = state.repo.create_abi(&payload.name, &abi_json.to_string()).await?;

    Ok((StatusCode::CREATED, Json(json!({ "abi": abi }))))
}
...
pub async fn get_abi_by_name(
    ...
    let abi = state.repo.get_abi(&abi_name).await?;

    Ok((StatusCode::OK, Json(json!({ "abi": abi }))))
}
