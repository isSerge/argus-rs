...
//! webhook-based channels based on trigger configurations. It acts as the
...
//!    of the trigger to be executed and a set of variables for template
//!    substitution.
...
//!    specific trigger configuration (e.g., `SlackConfig`) into a generic set
//!    of components needed to send a webhook.
//! 5. An HTTP client is retrieved from the `HttpClientPool`, configured with
//!    the appropriate retry policy for the specific trigger.
...
/// A trait to convert a specific trigger configuration (e.g., `SlackConfig`)
...
trait AsWebhookComponents {
    /// Transforms a specific trigger configuration into the generic components
...
impl AsWebhookComponents for TriggerTypeConfig {
    fn as_webhook_components(&self) -> Result<WebhookComponents, NotificationError> {
        let (url, message, method, secret, headers, builder, retry_policy): (
            ...
            TriggerTypeConfig::Discord(DiscordConfig { discord_url, message, retry_policy }) => (
                discord_url.clone(),
                message.clone(),
                Some("POST".to_string()),
                None,
            ...
            TriggerTypeConfig::Slack(SlackConfig { slack_url, message, retry_policy }) => (
                slack_url.clone(),
                message.clone(),
                Some("POST".to_string()),
                None,
...
impl NotificationService {
    ...
    /// # Arguments
    ///
    /// * `triggers` - A vector of `TriggerConfig` loaded and validated at
    ///   application startup.
    ...
    pub fn new(triggers: Vec<TriggerConfig>, client_pool: Arc<HttpClientPool>) -> Self {
        ...
        NotificationService { client_pool, triggers, template_service: TemplateService::new() }
    }

    /// Executes a notification for a given trigger.
    ///
    /// This method looks up the trigger by name, constructs the necessary
    ...
    ///   `NotificationError` if the trigger is not found, the HTTP client
    ...
    /// and executing notifications based on the configured triggers.
