...
async fn test_success() {
    ...
    let mock_discord_trigger = TriggerConfig {
        ...
        config: TriggerTypeConfig::Discord(DiscordConfig {
            ...
            message: NotificationMessage {
                ...
                body: "Monitor {{ monitor_id }} triggered on block {{ block_number }}".to_string(),
            },
    ...
    let mock = server
        ...
        .with_body(r#"{"content":"*Test Title*\n\nMonitor 1 triggered on block 123"}"#)
        .create_async()
        .await;

    let http_client_pool = Arc::new(HttpClientPool::new());
    let notification_service =
        NotificationService::new(vec![mock_discord_trigger], http_client_pool);
    ...
    let monitor_match = MonitorMatch {
        ...
        trigger_name: "test_discord".to_string(),
        trigger_data: json!({}),
        log_index: None,
    };
...
async fn test_failure_with_retryable_error() {
    ...
    let monitor_match = MonitorMatch {
        monitor_id: 2,
        block_number: 456,
        trigger_name: "test_discord_retry".to_string(),
        transaction_hash: Default::default(),
        contract_address: Default::default(),
        trigger_data: json!({}),
        log_index: None,
...
async fn test_failure_with_non_retryable_error() {
    ...
    let monitor_match = MonitorMatch {
        monitor_id: 3,
        block_number: 789,
        trigger_name: "test_discord_no_retry".to_string(),
        transaction_hash: Default::default(),
        contract_address: Default::default(),
        trigger_data: json!({}),
        log_index: None,
...
async fn test_failure_with_invalid_url() {
    ...
    let monitor_match = MonitorMatch {
        monitor_id: 4,
        block_number: 101,
        trigger_name: "test_invalid_url".to_string(),
        transaction_hash: Default::default(),
        contract_address: Default::default(),
        trigger_data: json!({}),
        log_index: None,
